...
export default function EditorEmpresa() { 
  ...
  const agregarCaracteristica = () => {
    ...
    setProductoEnEdicion(prev => ({ ...prev, caracteristicas: [...prev.caracteristicas, nuevaCaracteristica.trim()] }));
          ...
          <div className="w-full bg-orange-50 border border-orange-200 rounded-[24px] p-5 md:p-6 flex flex-col md:flex-row items-center justify-between gap-5 shadow-sm animate-in fade-in slide-in-from-top-4 z-10">
            <div className="flex items-center gap-4 text-left w-full md:w-auto">
              ...
              <div>
                ...
                <p className="text-sm text-orange-700/90 font-medium mt-0.5 leading-snug">Tu perfil de proveedor no está visible en el directorio. Regularizá tu situación para volver a aparecer.</p>
              </div>
            </div>
            <button
                ...
                <div className="w-full bg-white rounded-[32px] shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row items-center p-6 gap-6">
                  ...
                  <div className="flex-1 text-center md:text-left min-w-0">
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-3 text-xs font-medium text-gray-500">
                      <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-[#2D6A6A]" /> {formData.zonaCobertura.length > 0 ? `${formData.zonaCobertura.length} zonas de cob.` : "Ubicación a definir"}</span>
                      ...
                      <div className="flex flex-col sm:flex-row gap-6 mb-8 mt-2 md:mt-0">
                        ...
                        <div className="flex-1 relative group cursor-pointer text-left">
                          <div onClick={() => triggerFileInput(bannerInputRef)} className={`w-full h-32 rounded-[28px] overflow-hidden border-2 border-dashed ${formData.banner ? 'border-transparent' : 'border-gray-200'} transition-all flex items-center justify-center bg-gray-50 block cursor-pointer relative group/img`}>
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/img:opacity-100 flex items-center justify-center transition-opacity"><span className="text-white font-bold text-xs uppercase tracking-widest flex items-center gap-2"><Camera className="w-5 h-5"/> Cambiar Portada</span></div>
                          </div>
                    ...
                    <Accordion title="Ubicación, Contacto y Redes" icon={MapPin} isOpen={openSection === 'contacto'} onToggle={() => setOpenSection(openSection === 'contacto' ? null : 'contacto')}>
                      ...
                      <div className="pt-2 border-t border-gray-100">
                         ...
                         <div className="mt-2 mb-6">
                           <label className="flex items-center text-xs font-bold text-gray-500 uppercase tracking-widest leading-none mb-4 ml-1">Modalidad de Atención</label>
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {[
                                { id: 'ventaOnline', label: 'Venta Online 24/7' },
                                { id: 'showroom', label: 'Showroom con cita previa' },
                                { id: 'local', label: 'Local a la calle' },
                                { id: 'domicilio', label: 'Atención a domicilio / Visitas' }
                              ].map(item => (
                                <div key={item.id} className="flex items-center gap-3 cursor-pointer group w-full" onClick={() => handleNestedChange('modalidad', item.id)}>
                                  <div className={`w-5 h-5 rounded-[6px] border flex items-center justify-center transition-colors shrink-0 ${formData.modalidad[item.id] ? 'bg-[#4DB6AC] border-[#4DB6AC]' : 'bg-white border-gray-300 group-hover:border-[#4DB6AC]'}`}>
                                    {formData.modalidad[item.id] && <Check className="w-3.5 h-3.5 text-white" />}
                                  </div>
                                  <span className={`text-[13px] font-bold ${formData.modalidad[item.id] ? 'text-[#1A3D3D]' : 'text-gray-500'}`}>{item.label}</span>
                                </div>
                              ))}
                           </div>
                         </div>
                      ...
                      <div className="pt-2 border-t border-gray-100">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 items-end">
                            <InputGroup label="WhatsApp Ventas (Sin '+')" id="whatsappVentas" value={formData.whatsappVentas} onChange={handleChange} required tooltip="Número principal para cotizaciones." />
