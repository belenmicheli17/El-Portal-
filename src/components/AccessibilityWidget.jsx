...
const location = useLocation();
  ...
  useEffect(() => {
   const abrirAccs = () => { setIsOpen(false); };
...
return () => {
  ...
  useEffect(() => {
    ...
    if (FONT_SIZES[settings.fontSizeIndex] === 'grande') document.body.classList.add('font-size-grande');
  ...
  useEffect(() => {
    ...
    if (settings.readingGuide) window.addEventListener('mousemove', handleMouseMove);
  ...
  const resetSettings = () => {
    setSettings({ fontSizeIndex: 0, grayscale: false, dyslexiaFont: false, highlightLinks: false, stopAnimations: false, readingGuide: false });
  };
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.id = styleId;
      style.innerHTML = `
        /* Estilos CSS originales omitidos para no hacer largo el bloque, 
           MANTÉN TUS ESTILOS EXACTAMENTE IGUAL AQUÍ */
        ...
        body.font-size-grande nav { align-items: center !important; }
        body.font-size-grande nav div { align-items: center !important; }
        ...
        body.font-size-grande nav [class*="text-2xl"] { font-size: 24px !important; }
        ...
        body.font-size-grande .accessibility-widget-container div { line-height: normal !important; }
        ...
        body.dyslexia-font-active, body.dyslexia-font-active * { font-family: 'OpenDyslexic', 'Comic Sans MS', sans-serif !important; }
        body.links-highlighted a { 
          background-color: #FFFF00 !important; color: #000000 !important; text-decoration: underline !important;
          outline: 3px solid #FFD700 !important; font-weight: 800 !important;
        }
        body.stop-animations-active * { animation: none !important; transition: none !important; }
      `;
  ...
  // ==========================================
  // CÁLCULO DE DIRECCIÓN DE APERTURA
  // ==========================================
  // Si está de la mitad para arriba de la pantalla, se abre hacia abajo. Si no, hacia arriba.
  ...
  return (
    <>
      {settings.readingGuide && (
        <div className="fixed left-0 w-full h-1 bg-red-500 z-[10001] pointer-events-none shadow-[0_0_10px_rgba(239,68,68,0.5)] hidden md:block" style={{ top: `${readingGuideY}px` }}></div>
      )}
      {settings.grayscale && (
        <div className="fixed inset-0 pointer-events-none z-[9998]" style={{ backdropFilter: 'grayscale(100%)', backgroundColor: 'transparent' }}></div>
      )}

      <div 
        className={`accessibility-widget-container fixed z-[10000] transition-none 
          ${!position ? (hasCookiesBanner ? 'bottom-40 md:bottom-28 left-5' : 'bottom-5 left-5') : ''}
        `}
        ref={widgetRef}
        style={{
      ...
      >
        {isOpen && (
          <div 
            className={`absolute w-[calc(100vw-40px)] md:w-80 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.25)] border border-gray-100 p-5 animate-in fade-in duration-300 text-left overflow-y-auto max-h-[70vh]
              ${isTopHalf ? 'top-[calc(100%+16px)]' : 'bottom-[calc(100%+16px)]'}
          ...
          >
            <div className="flex justify-between items-center mb-5 pb-2 border-b border-gray-100">
              ...
              <button onClick={resetSettings} className="text-[10px] font-bold text-red-500 hover:text-red-700 flex items-center gap-1 transition-colors px-2 py-1 bg-red-50 rounded-lg shrink-0">
